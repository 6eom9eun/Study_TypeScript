function 함수1_5(x :number | string){ // 타입이 아직 하나로 확정되지 않았을 경우 Tpye Narrowing 사용
...
function 함수1_5_1(x :number | string){ // 타입이 아직 하나로 확정되지 않았을 경우 Tpye Narrowing 사용
    let array :number[] = [];
...
function 함수1_5_2(x :number | string){ 
    let array :number[] = [];
    array[0] = x as number; // x를 number로 강제 지정, if문 필요없음
}

let 이름1_5 :string = 'kim';
